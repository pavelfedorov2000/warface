...
interface Props extends ChatMessageType {
    index: number;
}

const ChatMessage = ({ index, date, text }: Props) => {
    return (
        <div className={classNames('chat-message', {
            'is-user': index % 2 !== 0,
            'is-support': index % 2 === 0,
        })}>
            <p className="chat-message__text">{text}</p>
            <time dateTime={formatDateTime(date)} className="chat-message__date">
                <img src={calendarIcon} alt="calendar" />
                <span>{date}</span>
            </time>
            <button className="chat-message__remove">
